...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def testrun(action):
        ...
        ball1.bounce_racket()
        count_bounce, hit_cards = ball1.fly(tick_step, list(tb_cards))
        ...
        if op_card[1] != CARD_SPIN:
            for acc in range(vbond[1] - vel0 + 1, vbond[0] - vel0 - 1):
                if evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code) < costmin:
                    accmin = acc
                    cardmin = card_using
                    costmin = evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code)
            ...
            for acc in range(((vbond[1] - vel0) // CARD_SPIN_PARAM).__int__() + 1, ((vbond[0] - vel0) // CARD_SPIN_PARAM).__int__() - 1):
                if evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code) < costmin:
                    accmin = acc
                    cardmin = card_using
                    costmin = evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code)
            for acc in range(((vbond[3] - vel0) // CARD_SPIN_PARAM).__int__() + 1, ((vbond[2] - vel0) // CARD_SPIN_PARAM).__int__() - 1):
                if evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code) < costmin:
                    accmin = acc
                    cardmin = card_using
                    costmin = evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, 0, card_code)
