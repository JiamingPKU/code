# PREFACE：
# 本代码基于Yusen Zhu的基础框架和基本思路，并且参考和部分融合了小组成员的想法；
# 代码为SWIFT系列，version == 12；并通过algo系列、RAW系列进行测试；
# Guanli Wang做了代码外观和参数的调整以及细节的优化；
# Yusen Zhu，V11加入了根据对方跑位更改评估函数中对对方跑位的猜测以及第二生命阈值
# 最后更新：7:07 p.m., 5 Jun, 2017
...
card_value_menu = {'SP': 2000, 'DS': 10, 'IL': 4000,
                   'DL': 4010, 'TP': 400, 'AM': 2000}   # 道具价值原始列表（会动态调整）
run_menu = {}     # 跑位模式原始对应表
memory = {}     # 单局对战记录表
...
def vel_critical(pos):
    up_ub = int((DIM[3] * 3 - pos) / tick_step) - \
        1                  # bounces = 2
    up_mid = int((DIM[3] * 2 - pos) / tick_step) + \
        1                 # bounces = 2
    up_lb = int((DIM[3] - pos) / tick_step) + \
        1                      # bounces = 1
    down_lb = -int((DIM[3] * 2 + pos) / tick_step) + \
        1               # bounces = 2
    down_mid = -int((DIM[3] + pos) / tick_step) - \
        1                  # bounces = 2
    # bounces = 1
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def test(action):
        test_ball = Ball(DIM, copy.deepcopy(
            tb.ball['position']), copy.deepcopy(tb.ball['velocity']))
        test_ball.update_velocity(
            action.acc, copy.deepcopy(tb.op_side['active_card']))
    ...
    def hit_corners():
        ...
        for i in range(0, 2):
            for j in range(0, 3):
                new_action = RacketAction(tb.tick, bat, crit[i][
                                          j] - tb.ball['velocity'].y, None, None, None)
                if tb.op_side['active_card'][1] == 'SP':
                    new_action.acc = int(new_action.acc / CARD_SPIN_PARAM)
                new_route = Route(tb.ball['position'].y, crit[i][
                                  j], new_action, 'HIT_CORNERS')
    ...
    def hit_cards():
        for card in copy.deepcopy(tb.cards['cards']):
            ...
            for vel in vel_possible:
                new_action = RacketAction(
                    tb.tick, bat, vel - tb.ball['velocity'].y, None, None, None)
                if tb.op_side['active_card'][1] == 'SP':
                    new_action.acc = int(new_action.acc / CARD_SPIN_PARAM)
                new_route = Route(
                    tb.ball['position'].y, vel, new_action, 'HIT_CARDS')
    ...
    class Rally:
        ...
        def update_info(self, tb_current, param):
            if param == 'INITIAL':
                ...
                self.op_side_info[
                    'card_used'] = tb_current.op_side['active_card']
                self.op_side_info[
                    'current_cards'] = tb_current.op_side['cards']
                self.op_side_info[
                    'pos_initial'] = tb_current.op_side['position']
            elif param == 'FINAL':
                ...
                self.outcome = (self.op_side_info['life_final'] - self.op_side_info['life_initial'])\
                    - (self.side_info['life_final'] -
                       self.side_info['life_initial'])
    ...
    def check(route):
        ...
        for card in tb.op_side['cards']:
            if card.code == 'DL':
                ...
                break
        if LIFE_LIMIT_2 < life < LIFE_LIMIT_1:
    ...
    def get_run_strategy(route, history):
        ...
        # 我方击球位置太靠近中间时；
        # 统计的acc_cost未给出明显倾向时
        # if LIFE_LIMIT_2 < tb.side['life'] < LIFE_LIMIT_1 or not
        # is_atcorner(route.start):
        ...
        for card in tb_cards:
            if card.code == 'IL' or card.code == 'DL':
                if CARD_EXTENT[1] - abs(card.pos.x) <= 50000:
                    if card.pos.y - CARD_EXTENT[2] <= 50000 or CARD_EXTENT[3] - card.pos.y <= 50000:
                        route.action.run = (card.pos.y - route.start) // 2
                        print('Test')
                        check(route)
                        return

        # 正常打角情况（结合动态统计结果）
        if memory['op_acc_cost_live'] <= 300:       # 统计结果指示对方倾向于以acc_cost = 0的模式回球
            if route.start > DIM[3] // 2:
                run = DIM[3] // 4 * 3 - route.start
            else:
                run = DIM[3] // 4 - route.start
        # 统计结果指示对方倾向于以acc_cost = 700+的模式回球
        elif 500 <= memory['op_acc_cost_live'] <= 900:
            if route.start > DIM[3] // 2:
    ...
    def evaluate(route):
        ...
        if tb.op_side['run_vector'] is None or (aim_pos - tb.op_side['position'].y) * tb.op_side['run_vector'] > 0:
            try:
                ...
                if abs(op_run) > 240000:
                    op_run_cost = (
                        aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2 / 4
                    op_bat_cost = (
                        aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2 / 4
                else:
                    op_run_cost = (
                        aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2 // 16
                    op_bat_cost = (
                        aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2 * 9 // 16
            except:
                op_run_cost = (
                    aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2 / 4
                op_bat_cost = (
            ...
            op_run_cost = 0
            op_bat_cost = (
                aim_pos - tb.op_side['position'].y) ** 2 // FACTOR_DISTANCE ** 2
        ...
        if not isLegal(aim_pos, op_ball_vel):
            if op_ball_vel >= op_vel_range[0][2]:
                op_acc_cost = (
                    op_ball_vel - op_vel_range[0][2]) ** 2 // FACTOR_SPEED ** 2
            elif op_ball_vel <= op_vel_range[1][0]:
                op_acc_cost = (
                    op_ball_vel - op_vel_range[1][0]) ** 2 // FACTOR_SPEED ** 2
            elif 0 < op_ball_vel <= op_vel_range[0][0]:
                op_acc_cost = (
                    op_ball_vel - op_vel_range[0][0]) ** 2 // FACTOR_SPEED ** 2
            elif op_vel_range[1][2] < op_ball_vel < 0:
                op_acc_cost = (
                    op_ball_vel - op_vel_range[1][2]) ** 2 // FACTOR_SPEED ** 2
        cost -= op_acc_cost
        ...
        for card in route.getCards():
            # 获取道具的路线的落点靠近中间时，降低道具的获取价值（容易得不偿失）
            if not is_atcorner(route.getEnd()):
        ...
        if card_used == '':   # 不使用道具的策略也赋以一定价值
            ...
            cost -= memory['op_acc_cost_live'] * \
                3      # 用memory记录的对手的动态加速花费评估SP使用效果
        elif card_used == 'IL':
            cost -= 2000
            ...
            cost -= run_cost
            new_run_cost = max(route.action.run - CARD_TLPT_PARAM,
                               0) ** 2 // FACTOR_DISTANCE ** 2
            cost += new_run_cost
        elif card_used == 'AM':
            cost -= op_bat_cost
            cost -= op_acc_cost
        ...
        # 斩杀系统（结合对手的最大移动距离）
        op_max_distance = tb.op_side['life'] / \
            RACKET_LIFE * BALL_V[0] * tick_step
    ...
    def update_memory():
        ...
        if pre_turns >= 1:
            ...
            op_acc = vel0 - vel
            # 获取上一局对手的加速消耗
            memory['op_acc_cost_live'] = op_acc ** 2 // FACTOR_SPEED ** 2

            # 根据上次的acc生成对方本轮跑位所可能依据的tag
            tag = "item"
            op_dis1 = pre_rally.op_side_info[
                'pos_final'].y - pre_rally.op_side_info['pos_initial'].y
            op_dis1 = abs(op_dis1)
            ...
            if pre_turns > 1:
                ...
                op_dis = pre2_rally.op_side_info[
                    'pos_final'].y - pre2_rally.op_side_info['pos_initial'].y
                op_dis = abs(op_dis)
                if op_dis > DIM[3] * 0.9:
                    tag_run = 'far_end'
                elif op_dis < DIM[3] * 0.1:
                    tag_run = 'near_end'

            # 通过对方总消耗减去其他消耗算出对方跑位
            op_batacc_cost = pre_rally.op_side_info[
                'life_initial'] - pre_rally.op_side_info['life_final']
            op_batacc = pre_rally.op_side_info[
                'pos_final'].y - pre_rally.op_side_info['pos_initial'].y
            if len(pre_rally.op_side_info['card_used']) > 0:
                ...
                if (pre_rally.op_side_info['card_used'])[1] == CARD_TLPT:
                    pass
            op_run = abs(op_batacc) - abs(2 * op_batacc_cost *
                                          FACTOR_DISTANCE ** 2 - op_batacc ** 2) ** 0.5
            op_run /= 2
            if tag_run == 'far_end':
