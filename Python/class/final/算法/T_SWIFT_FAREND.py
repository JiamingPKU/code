...
card_value_menu = {'SP': 200000, 'DS': 10, 'IL': 400000, 'DL': 401000, 'TP': 400, 'AM': 200000}   # 道具价值原始列表（会动态调整）
run_menu = {}     # 跑位模式原始对应表
memory = {}     # 单局对战记录表
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def get_run_strategy(route, history):
        ...
        # 正常打角情况（结合动态统计结果）
        elif memory['op_acc_cost_live'] <= 300:       # 统计结果指示对方倾向于以acc_cost = 0的模式回球
            if route.start > DIM[3] // 2:
                run = DIM[3] // 4 * 3 - route.start
            else:
                run = DIM[3] // 4 - route.start
        elif 500 <= memory['op_acc_cost_live'] <= 900:  # 统计结果指示对方倾向于以acc_cost = 700+的模式回球
            if route.start > DIM[3] // 2:
        ...
        acc_cost = (route.acc) ** 2 // (FACTOR_SPEED ** 2)
        run_cost = run ** 2 // (FACTOR_DISTANCE ** 2)
        ...
        if LIFE_LIMIT_2 < life < LIFE_LIMIT_1:
            ...
            max_dist -= 10000
            route_end = route.start + run
            if route_end > max_dist:
                run = max_dist - route.start
            elif DIM[3] - route_end > max_dist:
                run = DIM[3] - max_dist - route.start
    ...
    def evaluate(route):
        ...
        cost -= 100 * int(op_run_cost + op_bat_cost)
    ...
    def update_memory():
        ...
        if pre_turns >= 1:
            ...
            op_batacc_cost = pre_rally.op_side_info['life_initial'] - pre_rally.op_side_info['life_final']
            op_batacc = pre_rally.op_side_info['pos_final'].y - pre_rally.op_side_info['pos_initial'].y
            if len(pre_rally.op_side_info['card_used']) > 0:
                ...
                if (pre_rally.op_side_info['card_used'])[1] == CARD_TLPT:
                    pass
            op_run = abs(op_batacc) - abs(2 * op_batacc_cost * FACTOR_DISTANCE ** 2 - op_batacc ** 2) ** 0.5
            op_run /= 2
