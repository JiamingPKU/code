...
*** pooled 估计
* robust回归
reg govsize open lngdppc lnpop anr p1-p27, robust
est store est1
outreg2 using est, tex replace ctitle(pool robust) drop(p* o.p*)
* cluster回归
reg govsize open lngdppc lnpop anr p1-p27, vce(cluster prov)
est store est2
outreg2 using est, tex append  ctitle(pool P_clus) drop(p* o.p*)
* cluster回归
reg govsize open lngdppc lnpop anr p1-p27, vce(cluster year)
est store est3
outreg2 using est, tex append  ctitle(pool Y_clus) drop(p* o.p*)


*** 面板估计,固定效应模型
*	同时加入了省份固定效应,以及考虑到异方差的问题
xtreg govsize open lngdppc lnpop anr p1-p27, fe vce(cluster prov)
est store est4
outreg2 using est, tex append	ctitle(FE-i P_clus) drop(p* o.p*)

*	横向和纵向的固定效应
xtreg govsize open lngdppc lnpop anr p1-p27 y1-y8, fe vce(cluster prov)
est store est5
outreg2 using est, tex append	ctitle(FE-it P_clus) drop(p* o.p* y* o.y*)

/* 
*	随机效应模型,考虑到现实意义,舍去
xtreg govsize open lngdppc lnpop anr p1-p27, re vce(cluster prov)
est store est4
outreg2 using est, tex append
*/
