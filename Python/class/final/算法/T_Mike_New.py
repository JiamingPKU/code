...
# 代码为SWIFT系列，version == 10；并通过algo系列、RAW系列进行测试；
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def evaluate(route):
        ...
        if life > LIFE_LIMIT_1:
            life = life - acc_cost - run_cost - bat ** 2 // FACTOR_DISTANCE ** 2
            for card in tb.op_side['cards']:
                if card.code == 'DL':
                    life -= CARD_DECL_PARAM
                    break
            if life < LIFE_LIMIT_1:
                if route.action.run == DIM[3] // 4 * 3 - route.start:
                    route.action.run = DIM[3] // 8 * 5 - route.start
                elif route.action.run == DIM[3] // 4 - route.start:
                    route.action.run = DIM[3] // 8 * 3 - route.start
        elif life > LIFE_LIMIT_2:
            pass
