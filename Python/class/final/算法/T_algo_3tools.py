...
# 限制3个道具即开始使用。(line205)改变了赋值函数。如果道具达到上限，那么为了立即使用道具，将使用道具的成本变为0
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def evaluation(pos, vel, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, run, use_card):
        ...
        if use_card == 'TP':
            ...
            new_runcost = (max(run - CARD_TLPT_PARAM, 0) /
                           FACTOR_DISTANCE) ** 2
            # 如果道具达到上限，那么为了立即使用道具，将使用道具的成本变为0
            if len(cards) == 3:
                new_runcost = 0
            cost += new_runcost
            # life += run_cost
    ...
    for card_using in deck:
        ...
        if op_card[1] != CARD_SPIN:
            for acc in range(vbond[1] - vel0 + 1, vbond[0] - vel0 - 1):
                cost1 = evaluation(pos0, vel0, life, op_life, tb_cards,
                                   op_pos, op_run, op_card, cards, acc, run, card_code)
                if cost1 < costmin:
                    accmin = acc
                    cardmin = card_using
                    costmin = cost1
    ...
    #     card_code = card_using.code
    #     # 无道具模式
    #     paras = [pos0, vel0, life, op_life, (), op_pos, op_run, op_card, list(cards), (DIM[3] - DIM[2]) // 2 - pos0, card_code]
    #     if op_card[1] != CARD_SPIN:
    #         a1, costa1 = searchaccrange(vbond[1] - vel0 + 1, vbond[0] - vel0 - 1, step, paras)
    #         a2, costa2 = searchaccrange(vbond[3] - vel0 + 1, vbond[2] - vel0 - 1, step, paras)
    #     else:
    #         a1, costa1 = searchaccrange(((vbond[1] - vel0) // CARD_SPIN_PARAM).__int__() + 1,
    #                             ((vbond[0] - vel0) // CARD_SPIN_PARAM).__int__() - 1, step, paras)
    #         a2, costa2 = searchaccrange(((vbond[3] - vel0) // CARD_SPIN_PARAM).__int__() + 1,
    #                             ((vbond[2] - vel0) // CARD_SPIN_PARAM).__int__() - 1, step, paras)
    #     if costa1<costa2:
    #         if costmin>costa1:
    #             costmin=costa1
    #             accmin=a1
    ...
    #         velcards = [(yy/(abs(cardx)) * BALL_V[0]).__int__() for yy in ycards]
    #         acccards = [v - vel0 for v in velcards if issecure(pos0,v)]
    #         if len(acccards) >0:
    #             costs=[evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, (DIM[3] - DIM[2]) // 2 - pos0, card_code) for acc in acccards]
    ...
    # 学习
    pass
    return RacketAction(tb.tick, pos0 - tb.side['position'].y, accmin, (DIM[3] - DIM[2]) // 2 - pos0, None, cardmin)
