# to do: 1. 优化道具价值 3. 优化对方击球花费（传送卡） 4. 优化对方加速花费 6. 考虑道具容量
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def testrun(action):
        ...
        ball1.update_velocity(action.acc, tb.op_side['active_card'])
        ball1.bounce_racket()
    ...
    if op_card[1] != CARD_SPIN:
        for acc in range(vbond[1] - vel0 + 1, vbond[0] - vel0 - 1):
            if evaluation(pos0, vel0, tb_cards, op_pos, op_run, op_card, cards, acc, 0, '') < costmin:
                accmin = acc
                costmin = evaluation(pos0, vel0, tb_cards, op_pos, op_run, op_card, cards, acc, 0, '')
    ...
    # 学习
    pass
    return RacketAction(tb.tick, pos0 - tb.side['position'].y, accmin, (DIM[3] - DIM[2]) // 2 - pos0, None, None)
