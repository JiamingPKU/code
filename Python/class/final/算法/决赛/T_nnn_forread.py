...
def play(tb, ds):
    ...
    for i in (-1,):
        ...
        # 表示前一回合中对方消耗的体力
        dlife = (pr.op_side['life'] - cu.op_side['life']
                 ) if i != 0 else 100000 - cu.op_side['life']
        # v1表示对方接球时的速度
        temp, v1 = fly(act, pr.ball['position'].y, pr.ball[
                       'velocity'].y, pr.op_side['active_card'][1] == 'SP')
        # 直接将球原路接回的动作
        re_act = RacketAction(
            None, None, -2 * cu.ball['velocity'].y, None, None, None)
        # -v2表示对方接球后的速度
        temp, v2 = fly(re_act, cu.ball['position'].y, cu.ball[
                       'velocity'].y, cu.op_side['active_card'][1] == 'SP')
        opacc = -v2 - \
            v1 if pr.op_side['active_card'][
                1] != 'SP' else 2 * (-v1 - v2)  # 对方的加速度
        acclife = (abs(opacc) ** 2 // FACTOR_SPEED ** 2)
        # 按居中跑位计算的体力消耗
        batlife = (abs(cu.op_side['position'].y - 500000) ** 2 // FACTOR_DISTANCE ** 2) * (
            CARD_AMPL_PARAM if cu.op_side['active_card'][1] == 'AM' else 1)
        runlife = (abs(pr.op_side['position'].y - 500000)
                   ** 2 // FACTOR_DISTANCE ** 2)
        # if is_IL:  # 如果对方使用了补血包
        #     dlife += 2000
        ...
        # if abs(batlife + acclife + runlife - dlife) < 300:
        #     is_run = True
        # else:  # 否则不跑位
        #     is_run = False
        # # 判断对方是打双角还是打单角
    ...
    acc = 0
    run = (500000 - ball_pos.y) if optype in (-1, 1, 2, 3) else 0  # 跑位为居中或者不跑位
    move = 0.5 * (500000 - ball_pos.y)  # 不跑位时的跑位, 有点矛盾喵
    card_side = None
    card_used = None
    ...
    if is_AM or is_SP:
        ...
        if count_col(ball_v.y, ma, ball_pos.y, is_SP) in (1, 2):
            ...
            if run == 0:
                n = (life - 4000) / RACKET_LIFE
                maxbat = n * BALL_V[0] * tick_step
                if maxbat < max(DIM[3] - ball_pos.y, ball_pos.y - DIM[2]):
                    if DIM[3] - ball_pos.y > ball_pos.y - DIM[2]:  # 靠近下侧
                        run = (DIM[3] - ball_pos.y - maxbat) if abs(500000 - ball_pos.y) > abs(
                            DIM[3] - ball_pos.y - maxbat) else (500000 - ball_pos.y)
                    else:
                        run = (DIM[2] - ball_pos.y + maxbat) if abs(500000 - ball_pos.y) > abs(
                            DIM[2] - ball_pos.y + maxbat) else (500000 - ball_pos.y)
                else:
                    run = move
    ...
    for card in cards:
        ...
        for ac in eatcard(card_pos, ball_pos, ball_v.y, is_SP):
            if count_col(ball_v.y, ac, ball_pos.y, is_SP) in (1, 2):
                action = RacketAction(
                    tb.tick, bat, ac, (500000 - ball_pos.y), card_side, card_used)
    ...
    for tar in target_list:
        ...
        for ac in target(ball_pos.y, ball_v.y, tar.y, is_SP):
            if count_col(ball_v.y, ac, ball_pos.y, is_SP) in (1, 2):
                action_list.append(RacketAction(
                    tb.tick, bat, ac, run, card_side, card_used))
    ...
    if run == 0:
        ...
        if maxbat < max(DIM[3] - ball_pos.y, ball_pos.y - DIM[2]):
            if DIM[3] - ball_pos.y > ball_pos.y - DIM[2]:  # 靠近下侧
                run = DIM[3] - ball_pos.y - maxbat
            else:
                run = DIM[2] - ball_pos.y + maxbat
        else:
            run = move
        action.run = run
    return action
...
def summarize(tick: int, winner: str, reason: str, west: RacketData, east: RacketData, ball: BallData, ds: dict):
    new = {'tick': tick, 'winner': winner, 'reason': reason,
           'west': west, 'east': east, 'ball': ball}
    try:
...
def fly(action, ball_pos_y, ball_v_y, is_SP):
    ...
    if Y % DIM[3] != 0:
        count = Y // DIM[3]
        ball_pos_y = (Y - count * DIM[3]) * \
            (1 - 2 * (count % 2)) + DIM[3] * (count % 2)
    ...
    if optype in (0, 2):  # 如果对方打单角，得出相应的最优操作
        ...
        for tar in target_list:
            ...
            for ac in target(ball_pos_y, ball_v_y, tar.y, active_card == 'SP'):
                action_list.append(RacketAction(
                    None, bat, ac, run, None, None))
        for action in action_list:
            newtree = tree(action, active_card == 'SP',
                           active_card == 'AM', active_card == 'TP', True)
            op_ball_pos_y, op_ball_v_y = fly(
                action, ball_pos_y, ball_v_y, active_card == 'SP')
            tree_list.append(newtree)
    ...
    for tar in target_list:
        ...
        for ac in target(ball_pos_y, ball_v_y, tar.y, active_card == 'SP'):
            if count_col(ball_v_y, ac, ball_pos_y, active_card == 'SP') in (1, 2):
                action_list.append(RacketAction(
                    None, (ball_pos_y - pos.y) // 2, ac, (ball_pos_y - pos.y) // 2, None, None))
    if count_col(ball_v_y, 0, pos.y, active_card == 'SP') in (1, 2):  # 以及直接反弹回去
        action_list.append(RacketAction(
            None, (ball_pos_y - pos.y) // 2, 0, (ball_pos_y - pos.y) // 2, None, None))
    for action in action_list:
        newtree = tree(action, active_card == 'SP',
                       active_card == 'AM', active_card == 'TP', True)
        op_ball_pos_y, op_ball_v_y = fly(
...
class tree:  # 树节点
    ...
    def __init__(self, action, is_SP, is_AM, is_TP, is_self):
        action, eatedcard = action if isinstance(
            action, tuple) else (action, None)
