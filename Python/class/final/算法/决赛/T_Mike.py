...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def get_run_strategy(route, history):
        ...
        # 我方生命值低于设定阈值时；
        # 我方击球位置太靠近中间时；
        if LIFE_LIMIT2 < tb.side['life'] < LIFE_LIMIT or not is_atcorner(route.start):
        ...
        if test_rally.mode.traits == 'HIT_CARDS':
            ...
            return
        # 正常打角情况（结合动态统计结果）
        if memory['op_acc_cost_live'] <= 300:       # 统计结果指示对方倾向于以acc = 0的模式回球
            if route.start > DIM[3] // 2:
                run = DIM[3] // 4 * 3 - route.start
            else:
                run = DIM[3] // 4 - route.start
        elif 500 <= memory['op_acc_cost_live'] <= 900:  # 统计结果指示对方倾向于以acc = 700+的模式回球
            if route.start > DIM[3] // 2:
    ...
    def evaluate(route):
        ...
        if not isLegal(aim_pos, op_ball_vel):
            ...
            if 0 < op_ball_vel <= op_vel_range[0][0]:
                op_acc_cost = (op_ball_vel - op_vel_range[0][0]) ** 2 // FACTOR_SPEED ** 2
            if op_vel_range[1][2] < op_ball_vel < 0:
        ...
        if tb.side['life'] < LIFE_LIMIT:              # 我方生命值较低时，提高IL的获取价值
            card_value['IL'] += 5000
        ...
        if card_used == '':   # 不使用道具的策略也赋以一定价值
            ...
            cost -= 2000
            if tb.side['life'] < LIFE_LIMIT:             # 对方生命值较低时，提高DL的使用价值
                cost -= 2000
