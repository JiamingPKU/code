...
# (line239) 修改了跑位,如果生命值降低到跑不到3/4宽度的时候，那么跑位到中间
# (line231) 增加了3/4宽度生命值的计算函数
...
serve_para = 'upmax'
card_value = {'SP': 4000, 'DS': 10, 'IL': 4000, 'DL': 4010, 'TP': 400, 'AM': 2000}
# 使用道具花费
card_savevalue = {'SP': 1000, 'DS': -10, 'IL': 1970, 'DL': 1960, 'TP': 100, 'AM': 1000, '': 0}
# 搜索初始步长
step = 100
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def searchaccrange(start, stop, step, paras):
        ...
        if stop - start < 25:
            ...
            if abs(predictpos_back(p1,p2+accs[0])-p10)>DIM[3]/2:
                p10 = (DIM[3] - DIM[2])//2 - p1
    ...
    for card_using in deck:
        ...
        # print(card_code)
        # 不捡道具模式
        paras = [pos0, vel0, life, op_life, (), op_pos, op_run, op_card, cards, run, card_code]
        if op_card[1] != CARD_SPIN:
        ...
        for card in tb_cards:
            ...
            if op_card[1] == CARD_SPIN:
                acccards = [2 * v for v in acccards]
            if len(acccards) >0:
                costs=[evaluation(pos0, vel0, life, op_life, tb_cards, op_pos, op_run, op_card, cards, acc, run, card_code) for acc in acccards]
        ...
        # print(cardmin.code)
    pass
    if abs(predictpos_back(pos0, vel0+accmin)-run) > (DIM[3] - DIM[2])//2:
        run = (DIM[3] - DIM[2])//2 - pos0
    # # 暴力搜索跑位
    # runmin = 0
