...
# (line172)赋值函数进行了修改，去掉run作为参数输入，而是作为内部变量。同时在(line252)注释掉了跑位到中间的赋值。(line386)增加了返回结果前的run的计算。(line52)更改了secureVelocity函数,结果分别+/-3,采用了更加保守的策略。。
from table import *
import sys
import copy
...
def count_run(pos, vel):
    ...
    if abs(predictDistance) < 5:
        run = sign(predictDistance) * (-5)
    # 否则，跑位到预测落点的一半位置
    else:
        run = predictDistance // 2
    # 如果跑位距离为0
    pass
    return run
...
def secureVelocity(pos):
    up_ub = (DIM[3] * 3 - pos) // tick_step - 3
    up_lb = (DIM[3] - pos) // tick_step + 3
    down_lb = - ((DIM[3] * 2 + pos) // tick_step) + 3
    down_ub = - (pos // tick_step) - 3
