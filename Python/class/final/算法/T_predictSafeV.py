...
# 安全速度预测落点版本。修改包括： (line29)增加了count_run函数,计算安全速度返回的落点;
# (line172)赋值函数进行了修改，去掉run作为参数输入，而是作为内部变量。同时在(line252)注释掉了跑位到中间的赋值。(line386)增加了返回结果前的run的计算。对所有的evaluation函数，去掉run。对所有的searchacc函数，去掉p11。
from table import *
import sys
import copy
...
def play(tb: TableData, ds: dict) -> RacketAction:
    ...
    def searchaccrange(start, stop, step, paras):
        # 参数减少一个
        p1, p2, p3, p4, p5, p6, p7, p8, p9, p10 = paras
        if stop - start < 25:
            accs = list(range(start, stop + 1))
            costs = [evaluation(p1, p2, p3, p4, p5, p6, p7, p8,
                                p9, accforsearch, p10) for accforsearch in accs]
            mincost = min(costs)
        ...
        costs = [(evaluation(p1, p2, p3, p4, p5, p6, p7, p8, p9,
                             accforsearch, p10)) for accforsearch in accs]
    ...
    for card_using in deck:
        ...
        for card in tb_cards:
            ...
            if len(acccards) > 0:
                # 去掉run作为已知参数
                costs = [evaluation(pos0, vel0, life, op_life, tb_cards, op_pos,
                                    op_run, op_card, cards, acc, card_code) for acc in acccards]
